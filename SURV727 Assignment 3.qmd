...
historical_clean <- historical_raw[-11, ] |>
  ...
  select(year, population_count, change)
...
pages <- east_c
east_cols <- vector("list", length(pages))
...
for (i in seq_along(pages)) {
  ...
  yr_col  <- which(str_detect(nms, "^year$|year"))[1]
  if (is.na(yr_col)) yr_col <- 1L
  pop_col <- which(str_detect(nms, "^population$|pop|total"))[1]
  if (is.na(pop_col)) pop_col <- setdiff(seq_along(nms), yr_col)[1]
  dat <- data.frame(
    year = parse_number(as.character(tbl[[yr_col]])),
    population_count = parse_number(as.character(tbl[[pop_col]])))

  dat <- dat[!is.na(dat$year) & !is.na(dat$population_count), , drop = FALSE]
  dat <- dat[order(dat$year), , drop = FALSE]
  
  east_cols[[i]] <- dat$population_count[match(yrs, dat$year)]
}

col_names <- gsub(",_Chicago$", "", pages)
historical_extended <- cbind(historical_clean,
  setNames(as.data.frame(east_cols, check.names = FALSE), col_names))
